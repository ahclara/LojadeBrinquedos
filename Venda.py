...
class Venda:
    ...
    def adicionar_item(self, produto: Produto, quantidade: int) -> None:
        if produto.estoque >= quantidade:
            ...
            print(f"ERRO: Não foi possível adicionar {quantidade} de '{produto.nome}'. Estoque insuficiente.")

    def calcular_total(self) -> float:
        self.valor_total = sum(item.calcular_subtotal() for item in self.itens)
        print(f"Total da Venda {self.id_pedido} recalculado: R${self.valor_total:.2f}")
        return self.valor_total # float
